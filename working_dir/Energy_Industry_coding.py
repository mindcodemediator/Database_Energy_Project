...
def getSalesQuery:
    ...
    select * from
        (
        select county as "Location", state, year as "Year", trunc(sum(revenue)*1000/sum(mwh_sold), 3) as "Avg Rev($)/MWh Sold" from JOLIU.sales
        inner join JOLIU.territory
        using(util_id, year)
        where
            (
...
# read in sales query
dfSales = pd.read_sql(getSalesQuery(), con=connection, params=[state_select, county_select])
dfSales.columns = ('location', 'year', 'avg_rev')
# bar chart
st.write(dfSales)
bc = alt.Chart(dfSales).mark_bar().encode(
    x='year:O',
    y='avg_rev:Q',
    color='location:N'
    column='year:O'
...
def returnFuelCode(df, fuel_desc):
    return df.loc(fuel_desc, 'code')
...
def getFuelTrend():
    ...
    fuelTypes = st.multiselect('Select Fuel Type', dfFuelTypes['desc'], 'Solar')
    if(fuelTypes)
        fuelCode = returnFuelCode(fuelTypes)
    ...
    #query
    dfFuel = pd.read_sql(getFuelTrendQuery(), con = connection)
    dfFuel.columns = ('year', 'month', 'count')
